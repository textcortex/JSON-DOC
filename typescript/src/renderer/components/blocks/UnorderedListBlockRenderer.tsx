...
interface UnorderedListBlockRendererProps extends React.HTMLAttributes<HTMLUListElement> {
  block: any;
  depth?: number;
    ...
    >
      {block.children &&
        ...
        block.children.map((child: any, index: number) => (
          <BlockRenderer key={child.id || index} block={child} depth={depth} components={components} />
        ))}
    </ul>
  );
};
