...
async function splitReference() {
  ...
  console.log(`Creating ${segments} segments with 16:9 aspect ratio (${segmentWidth}x${segmentHeight})`);
  
  for (let i = 0; i < segments; i++) {
    const startY = i * segmentHeight;
    const actualHeight = Math.min(segmentHeight, height - startY);
    
    console.log(`Creating segment ${i + 1}/${segments} (y: ${startY}, height: ${actualHeight})`);
    
    const outputPath = path.join(splitDir, `reference_segment_${String(i + 1).padStart(2, '0')}.png`);
