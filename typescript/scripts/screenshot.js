...
const puppeteerPath = path.join(__dirname, '../node_modules/puppeteer/package.json');
if (!fs.existsSync(puppeteerPath)) {
  console.log('Installing puppeteer for screenshots...');
  exec('npm install puppeteer', { cwd: path.join(__dirname, '..') }, (error) => {
    if (error) {
...
async function takeScreenshots() {
  ...
  viewerContent = viewerContent.replace(/const PORT = [^;]+;/, `const PORT = ${PORT};`);
  ...
  try {
    ...
    console.log(`Creating ${segments} screenshot segments with 16:9 aspect ratio (${viewportWidth}x${segmentHeight})`);

    for (let i = 0; i < segments; i++) {
      const startY = i * segmentHeight;
      const actualHeight = Math.min(segmentHeight, fullHeight - startY);
      
      console.log(`Capturing segment ${i + 1}/${segments} (y: ${startY}, height: ${actualHeight})`);
      
      const screenshotPath = path.join(SCREENSHOT_DIR, `page_segment_${String(i + 1).padStart(2, '0')}.png`);
